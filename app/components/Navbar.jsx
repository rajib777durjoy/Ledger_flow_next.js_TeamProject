      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        <ul className="hidden md:flex items-center gap-1">
          {navLinks.map(({ href, label, badge }) => (
            <li key={href}>
              <Link href={href}
                className={`flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg text-[13.5px] font-medium transition-all ${
                  pathname === href
        ...
        <div className="hidden md:flex items-center gap-2">
          ...
          <Link href="/login"
            className="text-slate-300 border border-white/15 px-4 py-1.5 rounded-lg text-[13px] font-medium hover:bg-white/[0.07] hover:text-white transition-all">
            Sign in
          </Link>
          <Link href="/dashboard"
